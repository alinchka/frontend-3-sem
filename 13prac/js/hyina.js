...
document.querySelector('#sortBig').onclick = mySort;
document.querySelector('#sortSmall').onclick = mySort2;

function mySort(){
    let nav = document.querySelector('.cards');
    for (let i = 0; i < nav.children.length; i++){
      for (let j = i+1; j < nav.children.length; j++){
          if(+nav.children[i].getAttribute("card_price") < +nav.children[j].getAttribute("card_price")){
              replacedNode = nav.children[i]
              nav.children.appendChild(nav.children[i]);
              nav.replaceChild(nav.children[j], nav.children[i]);
              
              //insertAfter(nav.children[j], nav.children[i]);
            }
        }
    }
}
